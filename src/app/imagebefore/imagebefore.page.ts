...
@Component({
  selector: 'app-imagebefore',
  templateUrl: './imagebefore.page.html',
  styleUrls: ['./imagebefore.page.scss'],
})
export class ImagebeforePage implements OnInit {
  ...
  //View only photo
  viewPhoto(urlToShow: string){
    this.photoViewer.show(urlToShow);
  }
  ...
  async getBeforeImages() {
    ...
    //#### THIS IS STAFF ID
    dashboardPost.job_image_type = "before";
    ...
    .then( (result) => {
      ...
      const beforeImageCounter = this.dashboardProvider.returnResponseByNativeHttp(result)['result'][0]['data']['before'];
      if( this.dashboardProvider.returnResponseByNativeHttp(result)['result'][0]['data']['before'] === 'undefined' ) {
        this.beforeImageData = [];
