...
@Component({
  selector: 'app-imageafter',
  templateUrl: './imageafter.page.html',
  styleUrls: ['./imageafter.page.scss'],
})
export class ImageafterPage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      if (params && params.jobDataForAfterPage) {
        this.jobData = JSON.parse(params.jobDataForAfterPage);
      }
    });
  ...
  ngOnInit() {
    ...
    //this.jobData = ""; //this.navParams.get('jobDataForAfterPage');


    console.log('Job Id ' + this.jobData.jobID);

    //#### INVOKE DEFAULT IMAGE LIST
    this.getBeforeImages();
  }

    //#### SET UP ACTION TO LOAD DEFAULT AFTER IMAGES
    async getBeforeImages() {

      let loading = await this.loadingCtrl.create({message: 'Fetching images ... ', spinner: 'circles'});
      loading.present();
  
      //#### MAKE DEFAULT REQUEST FOR AFTER IMAGES IF WE HAVE
      ...
      //#### THIS IS STAFF ID
      dashboardPost.job_image_type = "after";
      ...
      .then( (result) => {
        ...
        const beforeImageCounter = this.dashboardProvider.returnResponseByNativeHttp(result)['result'][0]['data']['after'];
        if( this.dashboardProvider.returnResponseByNativeHttp(result)['result'][0]['data']['after'] === 'undefined' ) {
          this.beforeImageData = [];
    ...
    async uploadBefore() {
        ...
        let loading = await this.loadingCtrl.create({message: 'Uplaoding', spinner: 'circles'});
        loading.present();
    ...
    openImage(imgSrc) {
      //this.alertCtrl.create({title:'Open', buttons:['ok']}).present();
    }
